...
async fn reconstruct_secret_test(local_shares: Arc<Mutex<ShareStorage>>, main_collector: u32, protocol:u32,step:u32, party_id:u32, ports:Vec<u16>,addr: String) {
    ...
    let shares = storage.get_share(protocol, step, party_id);
    let share4send = shares.expect("share is None");
    ...
    while party_id != main_collector && sent_acks.len() == 1 {
        ...
        match send_message(&des_addr, my_addr.clone(), party_id as i32, 1, 0, share4send).await {
            Ok(_) => {
                ...
                sent_acks.insert(main_collector);
            }
            Err(e) => {
    ...
    if party_id == main_collector {
        println!("Start to show the shares stored in main collector...");
        let share0 = storage.show_shares();
    }
}
...
async fn receive_shares(addr: String, ports:Vec<u16>, party_id: usize, mut rx: mpsc::UnboundedReceiver<Message>) -> Arc<Mutex<ShareStorage>> {
        ...
        if msg.protocol == 1 { // Share protocol
            if msg.step == 0 { // Initial share
                println!("Party {} received shares from {}: ({},{})", 
                    party_id, msg.from_party_id, msg.share_1half, msg.share_2half);
...
async fn distribute_shares(addr: String, ports:Vec<u16>, party_id: usize) -> bool {
    let secret = 12345;
...
async fn main() {
    ...
    let receive_handle = tokio::spawn(async move {
         let local_shares_storage = receive_shares(addr_clone_for_receive, ports_clone_for_receive, party_id, rx).await;
         let _ = tx_local_shares.send(local_shares_storage);
     });
    ...
    let distribute_handle = tokio::spawn(async move {
         distribute_shares(addr_clone_for_distribute, ports_clone_for_distribute, party_id).await
    });
    println!("Waiting for local_shares_storage...");

    match rx_local_shares.await {
    Ok(local_shares_storage) => {
        println!("Received local_shares_storage, spawning reconstruct_handle...");
        let reconstruct_handle = tokio::spawn(async move {
            println!("Starting reconstruct_secret_test...");
            reconstruct_secret_test(
                Arc::clone(&local_shares_storage),
                0,
                1,
                0,
                party_id as u32,
        ...
        tokio::try_join!(
            ...
            (false, (), ()) // Ensure the return type matches the expected tuple
        });
    }
    Err(e) => eprintln!("Failed to receive local_shares_storage: {}", e),
    }
    // let local_shares_storage = rx_local_shares.await.expect("Failed to receive local_shares_storage");
    // let reconstruct_handle = tokio::spawn(async move {
    //     reconstruct_secret_test(
    //         Arc::clone(&local_shares_storage), 
    //         0, 
    ...
    //test_shares(addr, ports, party_id, rx).await;
